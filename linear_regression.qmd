...
1. **Causal inference**: You want to assess the effect of a change in one variable (typically called $X$) on another variable (often called $Y$).
2. **Prediction**: If you know the value(s) of variable(s) $X$, what is then the value of outcome variable $Y$?
...
Thus, finding (causal) mechanism helps the scientist or policy maker in assessing the outcomes of a particular (policy) intervention. In the (spatial-)economic realm one could think about trying to assess the following quantities:
...
#| eval: TRUE
cd  "/Users/tomba/projects/methods/"
```
...
3. Confidence intervals
    - This strategy estimates an interval for the **difference** of the mean test scores, so small versus large student-teacher ratio districts. This interval denotes the set of null-hypotheses that can **not** be rejected.
...
```{r fig.align='center', echo=FALSE, fig.cap = "Relation between the heigh fathers and the height of children", label='fig-galton2', out.width="400px"}
...
```{r fig.align='center', echo=FALSE, fig.cap = "Relation heigh fathers and height children", label='fig-galton1', out.width="400px"}
...
Similar to @sec-numevidence we have three strategies to make inferences:
...
In general, the population linear regression *model* is different as we add an element $u_i$.
\begin{equation}
...
Thus, estimation boils down to drawing a straight line through data---that is, for regression with only one regressor. But how to see this. Let's start with a couple observations from the Californian Testscore Dataset. These are plotted in @fig-observations. Here you four observations, where each observation consist of one independent variable ($X$) and one dependent variable ($Y$). These can also be seen as the combination of measurement and outcome. So, the first observation consist of the independent variable $X_1$ which equals a student teacher ratio of 16 and the dependent variable $Y_1$ which equals a test score of 682. The other observations have a similar interpretation. As mentioned above as well, this display with two axes and two variables is called a scatterplot and it is a very common way to visualize the combination of two variables.

```{r fig.align='center', echo=FALSE, fig.cap = "Displaying observations in a scatterplot", label='fig-observations', out.width="600px"}
knitr::include_graphics("./figures/observations.png")
```

Our aim is to draw a line through the data to estimate the population slope line. That means that with the sampled data at hand we would like know something about the underlying total population. @fig-unire displays that population regression line that we would like to estimate using our data. Note that the specification of the line here is $Y = \beta_0 + \beta_1 X$, where the parameters $\beta_0$ and $\beta_1$ are the population parameters; the parameters that we do not know and that we would like to estimate with data.

```{r fig.align='center', echo=FALSE, fig.cap = "The population regression line", label='fig-unire', out.width="600px"}
knitr::include_graphics("./figures/regression.png")
```

The way we estimate these parameters is displayed in @fig-unierror:

```{r fig.align='center', echo=FALSE, fig.cap = "Estimating a regression line in a scatterplot", label='fig-unierror', out.width="600px"}
knitr::include_graphics("./figures/regression_errors.png")
```

To estimate the population linear regression model we apply the ordinary least squares estimator. Again, as @fig-unierror shows as well, a linear regression line is a straight line through points in a scatterplot. Actually, we want to draw that line such that the distance of all points to that line is minimized. See that in @fig-unierror the distances between the points and the line are given by the $u_i$'s, the regression errors. So, if we somehow can minimize all $u_i$'s we are fine. But those distances could be both positive and negative and they might cancel each other out. Therefore, we first square the regression errors and then minimize (hence the name: ordinary least *squares*). Also, see from Eq. \ref{eq:ols} that:
...
In fact, the OLS estimators of the slope $\beta_1$ and the intercept $\beta_0$ are:^[This result is given but is not all too difficult to prove. However, usually you do need these types of equations in your work.]
...
Now, how to fill in predictions? One of the districts in the data set is Antelope (CA) for which $STR = 19.33$ and $TestScore = 657.8$ Then the predicted value for the testscore is $\hat{Y}_{Antelope}= 698.9 - 2.28 \times 19.33 = 654.8$ and the resulting residual is $\hat{u}_{Antelope} = 657.8 - 654.8 = 3.0$. In `STATA` you can retrieve all predictions from observations  

In `STATA` both the constant and the slope can be easily retrieved by invoking the **regress** command:

```{stata, collectcode=TRUE}
regress testscr str, robust
```

The coefficient of `STR` being $\hat{\beta}_1$ is now $-2.28$ and the constant being $\hat{\beta}_0$ equals $698.9$. We will discuss the rest of this output later. In `STATA` one can get all predictions for $\hat{Y}$ with the command **predict** after estimation. Note that here you need to give `STATA` a new variable name. 

```{stata, collectcode=TRUE}
predict y_hat
```

So, all prediction for $\hat{Y}$ are now stored with the variable name `y_hat`. This comes in very handy when drawing plots. To retrieve a specific $\hat{Y}_i$ for observation $i$, you need the margins command (also useful for other purposes). This boils down to:
```{stata, collectcode=TRUE}
margins, at(str = 19.33)
```
where we asked for the *marginal* value of only one value of `str`, namely evaluated at 19.33. Note that our value here of 654.9 deviates slightly from above because of approximation errors. 

#### Hypothesis testing {#sec-unitesting}

We can assess the importance of the line as well with hypothesis testing. Again, recall that in in applied econometrics we will only **reject** the **null**-hypothesis, we do not accept a scientific hypothesis based upon one statistical test only. So, we aim to test $H_0: E(Y) = \mu_{Y,0}$ vs. $H_1: E(Y) \neq \mu_{Y,0}$, where $\mu_{Y,0}$ is some pre-specified quantity that we are interested in. Typically $\mu_{Y,0} = 0$ as this denotes no relation, but sometimes you could be interested in, e.g., whether $\mu_{Y,0} = 1$ when testing elasticities. Or you could be interested in other quantities.
...
And that is a strange concept. You first imagine a world $H_0$ with the data that that world *should* provide and then test that imaginary world with your *actual* data at hand. .
...
So, if you know $\hat{\beta}_1$ and $\sigma_{\hat{\beta}_1}$ you can calculate this. However, computers are much faster, in doing so. For example, suppose we want to test whether $\beta_{1,0} = 0$ using the regression output displayed above which gives $\hat{\beta}_1 = -2.28$ and $\sigma_{\hat{\beta}_1} = 0.52$. That is step 1. Note that `STATA` already calculated the standard error of Eq. \ref{eq:olsse}. For the next step we need to compute the $t$-statistic, which is:
...
```{r fig.align='center', echo=FALSE, fig.cap = "Calculating the $p$-value of a two-sided test when $t^{act} = -4.38$", label='fig-pvalues', out.width="400px"}
...
In fact, there is a link between the $p$-value and the significance level. The significance level is pre-specified. For example, if the pre-specified significance level is 5\%, then you reject the null hypothesis if $|t| \geq 1.96$ or equivalently, you reject if $p \leq 0.05$. The $p$-value is sometimes called the marginal significance level. Often, it is better to communicate the $p$-value than simply whether a test rejects or not---the $p$-value contains more information than the "yes/no" statement about whether the test rejects.

But recently there has been some debate about using $p$-values [@amrhein2019scientists]. Why should you use a 5% significance level, what is so special about that number? Is it not better just to report coefficients and standard errors? @fig-significance shows a figure from the journal Nature and how scientists across all fields nowadays see $p$-values and statistical significance. This is not to say that statistical testing does not matter, but more the reporting of that statistical testing. First of all, $p$-values in themselves do not contain that much information. In the regression output of above the reported $p$-values are being equal to 0.000 which is not informative. Secondly, the cut-off point of 5% is a bit harsh and could lead to publications being published only with $p$-values just below 0.05, leading to a phenomenon that is called publication bias.
...
As stated at the start of @sec-problem applied econometrics focuses on finding a **causal** effect. But how do you do know that the $\hat{\beta}_1$ you estimate using the population regression model of Equation (\ref{eq:betacausal}) is indeed a causal effect. In other words, if you change $X_i$ with one unit, will $Y_i$ then change with $\beta_1$ **in reality**?
...
However, this does not automatically entail that the isolated observation should be deleted. What it does entail is that one should go back to the data and investigate whether the outlier could be a mistake---perhaps a typo made when preparing the data or something that went amiss when converting the data from an `Excel` format to a `STATA` format.
...
```{r fig.align='center', echo=FALSE, fig.cap = "Wages versus years of education", label='fig-wages', out.width="400px"}
...
3. So, you should **always** use heteroskedasticity-robust standard errors; that is, use the `, robust` option as we have done consistently in the regression above.
...
## Measures of fit {#sec-measuresoffit}
...
\begin{equation}
\text{R}^2= \frac{\text{ESS}}{\text{TSS}} = \frac{\sum^n_{i=1}\left(\hat{Y}_i - \overline{Y}\right)^2}{\sum^n_{i=1}\left(Y_i - \overline{Y}\right)^2}.
\label{eq:r2}
\end{equation}

To understand in a graph what this entails, we go back to our four observation scatterplot in @fig-rsquare and focus on the first observation. We add to this scatterplot a new horizontal dashed line which denotes the mean of test scores ($\bar{Y}$). Now the distance between the observation and the mean is given by $(Y_i - \bar{Y})$. If we square this we get the "total square" (TS). The squared distance between the population regression line and the observation itself yields the "residual square" $(Y_i - \hat{Y}_i)$ (RS). And finally the squared distance between the estimation and the mean gives the "explained square" $(\hat{Y}_i - \bar{Y})$ (ES). Doing this for all observations gives the totals (so TSS, RSS and ESS).

```{r fig.align='center', echo=FALSE, fig.cap = "Total, explained and residual variation", label='fig-rsquare', out.width="600px"}
knitr::include_graphics("./figures/r_square.png")
```
