...
Unfortunately, as already mentioned above, omitted variable bias occurs very often. So, how to correct for this such we can deal with this particular bias. In general, there are three strategies:

1. we can run a randomized controlled experiment in which treatment ($STR$) is randomly assigned: then percentage English learners ($PctEL$) is still a determinant of test scores, but by construction $PctEL$ should be uncorrelated with $STR$. Unfortunately, is it very difficult to randomize class size in reality and often this strategy is just not feasible as being too costly or unethical (this argument accounts by the way for all sciences, not only the social sciences);
...
Thus, if we have information about an important omitted variable, as in the case of the size of migrant communities in the example above, then we can use that information in a multivariate population regression model. In the case of two regressors, that would look like:
\begin{equation}
...
\end{equation}
And if we take the difference, then the interpretation of $\beta_1$ boils down again to the marginal effect:$\Delta Y = \beta_1 \Delta X_1$. Or, $\beta_1 = \frac{\Delta Y}{\Delta X_1}$ when holding $X_2$ constant and, likewise, $\beta_2 = \frac{\Delta Y}{\Delta X_2}$ when holding $X_1$ constant. Instead of taking changes you can as well take the first derivative of $X_1$ with respect to $Y$ which simply boils down to $\beta_1$. $\beta_0$ is now the predicted value of $Y$ when $X_1 = X_2 = 0$. 

If we do this for the the Californian school district data, then recall that the original population regression line was estimated as:
\begin{equation}
