...
This chapter is organised as follows. The next section addresses the problem of finding a *relation* between some $X$ and some $Y$. Here, we follow example in the well-known textbook of \@stock2003introduction where we look at the relation between school class size and school class performance. At the same time, we also introduce some `STATA` commands. At the same time this section deal as well with the statistical framework that is needed for applied econometrics. Note that we assume that the reader already had a course in introductionary statistics and provide only the basic concepts most important for this course in Appendix \@ref(appreviewstat). 
...
```{r, echo=FALSE, fig.cap = "A scatterplot with tests-cores on the $y$-axis and student-teacher ratio on the $x$-axis", label='scattercaschool'}
...
For estimation is rather straightforward as we need to calculate the *difference* between the mean test scores within each group, or:
...
in 1886, Galton started to research the height of adult children with the height of their parents [@galton1886regression]. The original data can be seen in the scatterplot in Figure @\ref(fig:galton2). What Galton expected was that the relation between the height of children and that of their parents was one-to-one. On average children should receive the same height of their parents. So, in fact he expected a $45^{\circ}$ line---the red line; a line with slope equal to 1. 
...
```{r, echo=FALSE, fig.cap = "Drawing a straight line through data in a scatterplot", label='unire'}
...
```{r, echo=FALSE, fig.cap = "Calculating the $p$-value of a two-sided test when $t^{act} = -4.38$", label='pvalues'}
...
```{r, echo=FALSE, fig.cap = "Critical review on the (mis)use of statistical significance", label='significance'}
...
```{r, echo=FALSE, fig.cap = "Condition mean independence assumption", label='ass1'}
...
```{r, echo=FALSE, fig.cap = "Effect of outliers on OLS estimations", label='outlier'}
...
```{r, echo=FALSE, fig.cap = "Homoskedastic standard errors", label='homoskedasticity'}
...
```{r, echo=FALSE, fig.cap = "Heteroskedastic standard errors", label='heteroskedasticity'}
...
```{r, echo=FALSE, fig.cap = "Wages versus years of education", label='wages'}
...
```{r, echo=FALSE, fig.cap = "Heteroskedasticity in Californation schools?", label='heteroskedasticityca'}
